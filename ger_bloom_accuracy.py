...
def analyze_grammatical_context(model, tokenizer, pronoun: str, case: str, ger_data: pd.DataFrame):
    ...
    relevant_sentences = ger_data[ger_data[case] == pronoun].iloc[:, 0].tolist()
...
def evaluate_case_prediction(model, tokenizer, ger_data: pd.DataFrame) -> dict:
    """
    ...
    # Add prediction pattern tracking
    ...
    for idx, row in ger_data.iterrows():
        try:
            ...
            # Update statistics
            total_predictions += 1
            case_accuracy[correct_case]['total'] += 1
            
            if predicted_case == correct_case:
                correct_predictions += 1
                case_accuracy[correct_case]['correct'] += 1
            
            # Track prediction patterns
            prediction_patterns[correct_case][predicted_case] += 1
            ...
            print(f"Sentence: {sentence if 'sentence' in locals() else 'N/A'}")
            print(f"Correct case: {correct_case if 'correct_case' in locals() else 'N/A'}")
            continue
        ...
        if case_accuracy[true_case]['total'] > 0:
            ...
            for pred_case, count in sorted_predictions:
                if count > 0:
                    ...
                    print(f"  Predicted as {pred_case}: {count} times ({percentage:.1f}%)")
...
# Load the German pronoun dataset
ger_data = pd.read_csv('ger_pronouns.csv', encoding='utf-8')

# Print DataFrame info for debugging
print("\nDataFrame columns:")
print(ger_data.columns.tolist())
print("\nFirst few rows:")
print(ger_data.head())
...
print("\nAnalyzing 'ihn' (he):")
analysis = analyze_pronoun_representation(model, tokenizer, "ihn", "Ich sehe")
if analysis is not None:
    ...
    print("\nComparing 'ihn' (he) with 'sie' (she):")
    similarities = compare_pronouns(model, tokenizer, "ihn", "sie")
    ...
    print("\nAnalyzing 'ihn' in nominative case contexts:")
    context_analyses = analyze_grammatical_context(model, tokenizer, "ihn", "acc", ger_data)
    print(f"Found {len(context_analyses)} contexts")
else:
    print("Analysis failed due to tokenization issues")

# Evaluate model's case prediction accuracy
print("\nEvaluating model's case prediction accuracy...")
evaluation_results = evaluate_case_prediction(model, tokenizer, ger_data)
...
with open('ger_bloom_accuracy.txt', 'w', encoding='utf-8') as f:
    f.write("BLOOM-3B German Case Prediction Evaluation Results\n")
    f.write("===============================================\n\n")
